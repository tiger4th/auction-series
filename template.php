...
<?php if($key < 2){ ?>
<?php echo trim($value, ' '); ?>
<?php if(end(array_keys($bread)) != $key){ ?>
 < 
<?php } ?>
<?php }elseif(end(array_keys($bread)) == $key){ ?>
... < <?php echo trim($value, ' '); ?>
<?php } ?>
<?php } ?>
...
<li><a href="http://tiger4th.com/auc/">　その他　</a></li>
</ul>
</div>
<!-- /PCヘッダ -->
...
<?php if (end(array_keys($bread)) == $key) { ?>
<li class="active"><?php echo trim($value, ' '); ?></li>
<?php } else { ?>
<li><a href="?q=&s=<?php echo $s; ?>&o=<?php echo $o; ?>&t=<?php echo $t; ?>&c=<?php echo $bread_id[$key]; ?>"><?php echo trim($value, ' '); ?></a> <span class="divider">></span></li>
<?php } ?>
<?php } ?>
</ul>
<!-- /パンくず -->
...
<li class="nav-header"><?php echo $category["Result"]["CategoryName"]; ?></li>
<?php foreach($category["Result"]["ChildCategory"] as $key => $value){ ?>
<?php if ($value["NumOfAuctions"] > 0) { ?>
<li<?php if($c==$value["CategoryId"]){ ?> class="active"<?php } ?>><a href="?q=&s=<?php echo $s; ?>&o=<?php echo $o; ?>&t=<?php echo $t; ?>&c=<?php echo $value["CategoryId"]; ?>"><?php echo $value["CategoryName"]; ?></a></li>
<?php } ?>
<?php } ?>
<?php if ($category["Result"]["Depth"] > 1) { ?>
<li><a href="?q=&s=<?php echo $s; ?>&o=<?php echo $o; ?>&t=<?php echo $t; ?>&c=<?php echo $category["Result"]["ParentCategoryId"]; ?>">上のカテゴリに戻る</a></li>
<?php } ?>
</ul>
</div><!--/.well -->
<?php } ?>
...
<li><a class="thumbnail" href="<?php echo $item_url[$val]; ?>" target="_blank"><div class="box center">
<div class="picture"> <img src="../img/nowloading.jpg" id="img_<?php echo $val; ?>" alt="<?php echo $title[$val]; ?>" /></div>
...
<li><div><a href="http://pinterest.com/pin/create/button/?url=<?php echo urlencode($this_url); ?>" class="pin-it-button" count-layout="horizontal"><img border="0" src="//assets.pinterest.com/images/PinExt.png" title="Pin It" /></a>&nbsp;</div></li>
